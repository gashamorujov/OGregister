...
const ExcelGrid = forwardRef(function ExcelGrid(
  ...
  const handleRowLongPress = useCallback((e, rowIdx) => {
    clearLongPress();
    const touch = e.touches?.[0];
    if (!touch || !onRowContextMenu) return;
    longPressTimerRef.current = window.setTimeout(() => {
      onRowContextMenu(
        rowIdx,
        Math.min(touch.clientX, window.innerWidth - 240),
        Math.min(touch.clientY, window.innerHeight - 180),
      );
      longPressTimerRef.current = null;
    }, 600);
  }, [clearLongPress, onRowContextMenu]);
          ...
          {visibleRows.map((row, vi) => {
            ...
            return (
              <div
                ...
                onTouchStart={(e) => handleRowLongPress(e, ri)}
