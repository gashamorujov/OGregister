...
export default function ImportExcelModal({ existingRows = [], onConfirm, onCancel }) {
  ...
  // "Mirror sync": when on, rows that exist on the site but are absent from
  // the imported source get deleted too, so the site ends up matching the
  // import exactly. Off by default — deleting is destructive, so the user
  // opts in per import rather than it happening silently.
  ...
  const processParsed = (records, matchedFields, fellBack) => {
    ...
    // Rows that exist on the site (with an identity key) but whose key
    // doesn't appear anywhere in the imported source — candidates for
    // deletion when "mirror sync" is turned on.
    const importedKeys = new Set(records.map((rec) => rowKey(rec)));
    const removed = (existingRows || []).filter((r) => {
      if (!r || !r._id) return false;
      const key = rowKey(r);
      if (!key || key === '|||') return false; // blank/template rows never count as "deleted"
  ...
  const resetOutcome = () => {
    ...
    setRemovedRows([]); setSyncDeletes(false);
  };
  ...
  const handleConfirm = () => {
    ...
    const removed = syncDeletes ? removedRows.map(r => r._id) : [];
    onConfirm({ added, updated, removed });
  };
            ...
            <>
              ...
              <div className="import-summary-stats">
                ...
                {removedRows.length > 0 && (
                  <div className="import-stat import-stat-warn"><b>{removedRows.length}</b><span>sətir mənbədə tapılmadı</span></div>
                )}
              </div>

              {removedRows.length > 0 && (
                <div className="import-note-warn import-sync-toggle">
                  <label className="import-sync-checkbox">
                    <input
                      type="checkbox"
                      checked={syncDeletes}
                    ...
                    />
                    <span>
                      <b>{removedRows.length}</b> sətir saytda var, amma idxal etdiyiniz mənbədə yoxdur (silinmiş ola bilər).
                      Bu qutunu işarələsəniz, təsdiqlədikdə bu sətirlər <b>saytdan da silinəcək</b> — sayt tam olaraq
                      idxal mənbəsi ilə eyniləşdiriləcək.
                    </span>
                  </label>
                  {syncDeletes && (
                    <div className="import-removed-list">
                      {removedRows.slice(0, 12).map((r) => (
                        <div key={r._id} className="import-removed-row">
                          {r.fullName || r.fullNameId || r.idNumber || r.serial || '(adsız sətir)'}
                        </div>
                      ))}
                      {removedRows.length > 12 && (
                        <div className="import-removed-more">+ daha {removedRows.length - 12} sətir</div>
                      )}
                    </div>
                  )}
                </div>
        ...
        <div className="modal-footer">
          ...
          {step === 'preview' && (
            <button
              ...
              disabled={newCount + updateCount === 0 && !(syncDeletes && removedRows.length > 0)}
              onClick={handleConfirm}
            >
              <CheckIcon /> Təsdiqlə ({newCount + updateCount}{syncDeletes && removedRows.length > 0 ? ` + ${removedRows.length} silinmə` : ''})
            </button>
          )}
        </div>
      </div>
